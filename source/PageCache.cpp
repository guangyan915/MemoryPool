...
Span* PageCache::NewSpan(size_t k)
{
	...
	if (k >= NPAGES)
	{
		...
		Span* span = new Span;
		span->_pageId = (ADDRES_TYPE)ptr >> PAGE_SHIFT;
		span->_n = k;
...
Span* PageCache::MapObjectToSpan(void* obj)
{
	...
	PageID id = (ADDRES_TYPE)obj >> PAGE_SHIFT;
...
size_t PageCache::GetIdToSize(PageID id) {
	return _idSizeMap.get(id);
}

void PageCache::SetIdToSize(PageID id, size_t size) {
	_idSizeMap.set(id, size);
}